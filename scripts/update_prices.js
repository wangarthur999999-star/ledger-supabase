...
export async function main() {
  ...
  for (const [symbol, cfg] of Object.entries(FRED_CONFIG)) {
    try {
      ...
      results.push({ symbol, price, unit: cfg.unit, source: 'fred', status: '✅' });
    } catch (err) {
      console.error(`   ❌ ${symbol}: ${err.message}`);
      errors.push({ symbol, source: 'fred', error: err.message });
    }
  }
  ...
  for (const [symbol, cfg] of Object.entries(METALS_DEV_CONFIG)) {
    try {
      ...
      results.push({ symbol, price: round4(spot.price), unit: cfg.unit, source: 'metals.dev', status: '✅' });
    } catch (err) {
      console.error(`   ❌ ${symbol}: ${err.message}`);
      errors.push({ symbol, source: 'metals.dev', error: err.message });
    }
  }

 ...
 *
 * @returns {number | null} 百分比 (2 位小数), 或 null 表示数据缺失/计算失败。
 *   - 真实 0%      -> 0
 *   - 首次写入    -> null  (UI 显示 "—")
 *   - 查询失败    -> null
 *   - 历史值为 0  -> null  (除以 0)
 *
 * 老 schema NOT NULL 不允许 null; 配合 migration 20260520000100_change_nullable.sql
 * 一起上线, 不能单独部署。
 */
async function computeChange(supabase, pair, newOfficial) {
  try {
    ...
    if (error) {
      console.warn(`⚠️ change 查询失败 ${pair} (${error.message}),写 null`);
      return null;
    }
    if (!data || data.length === 0) {
      console.log(`ℹ️ ${pair} 没有 20h 前的历史数据,change=null (首次或新币种)`);
      return null;
    }
    ...
    console.warn(`⚠️ change 计算异常 ${pair} (${err.message}),写 null`);
    return null;
  }
}

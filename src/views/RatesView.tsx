import { TrendingUp, TrendingDown, DollarSign, Euro, ArrowUpDown, WifiOff, RefreshCw, Loader2 } from "lucide-react";
  ...
  if (!isLoading && rates.length > 0) {
    ...
    lastUpdatedElement = (
      <p className={`text-xs font-bold mt-1 ${isStale ? 'text-red-600' : 'text-on-surface-variant/70'}`}>
        {isStale ? '🕒 ' : ''}{t('rates.lastUpdatedPrefix')}{text}
      </p>
    );
  }
          ...
          {rates.map((rate) => {
            ...
            return (
              <motion.div
                ...
                className="bg-surface-container-low rounded-3xl p-6 border border-surface-container shadow-sm overflow-hidden relative"
              >
